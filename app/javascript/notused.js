...
// ★ここに3つの関数を追加
  ...
  function updateMarkers(searchResults) {
    clearMarkers();
    ...
    searchResults.forEach((shop) => {
      ...
      const infoWindow = new google.maps.InfoWindow({
        ...
        <div class="p-3">
          ...
          <div class="mb-1 text-xs text-gray-600">カテゴリ：<span class="text-gray-600">${shop.category_name}</span></div>
          <div class="mb-3 text-xs text-gray-600">シーン：<span class="text-gray-600">${shop.scenes_name}</span></div>
          <button
            onclick="window.open('https://www.google.com/search?q=${encodeURIComponent(shop.title)}','_blank')"
            class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded text-xs w-full cursor-pointer"
          >
            Googleで検索
          </button>
        </div>
  ...
  fetch(`/shops.json?q[category_name_cont]=${encodeURIComponent(categoryName)}`)
